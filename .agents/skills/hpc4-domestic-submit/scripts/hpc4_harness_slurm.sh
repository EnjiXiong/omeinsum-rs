#!/usr/bin/env bash
# hpc4_harness_slurm.sh — mechanical driver for the /hpc4-domestic-submit skill.
#
...
# cluster profile (.agents/skills/hpc4-domestic-submit/profiles/active.toml or
...
PROFILE_DEFAULT=".agents/skills/hpc4-domestic-submit/profiles/active.toml"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SINFO_FMT='%P %a %.10l %.6D %.6t'
SACCT_FMT='JobID,State,ExitCode,MaxRSS,Elapsed'

die() { echo "hpc4_harness_slurm: $*" >&2; exit 1; }
...
profile_path() {
  ...
  if [[ -n "${HARNESS_CLUSTER_PROFILE:-}" ]]; then
    echo ".agents/skills/hpc4-domestic-submit/profiles/${HARNESS_CLUSTER_PROFILE}.toml"; return
  fi
...
cmd_probe_partitions() {
  ...
  printf 'PARTITION\tAVAIL\tTIMELIMIT\tNODES\tSTATE\n'
  remote "$alias" "sinfo -o '$SINFO_FMT'" | parse_sinfo
}

cmd_submit() {
  local array="" run_spec="" command="" entrypoint="" partition="" walltime="" cpus="" \
        script=".agents/skills/hpc4-domestic-submit/scripts/hpc4_array_sbatch.sh" extra=""
  while [[ $# -gt 0 ]]; do
    case "$1" in
...
cmd_classify() {
  ...
  printf 'cell_id\tstate\texit-code\tmaxRSS\telapsed\tclassification\n'
...
cmd_smoke_test() {
  local partition="" walltime="00:05:00" run="smoke" script=".agents/skills/hpc4-domestic-submit/templates/smoke_test.sbatch"
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --partition) partition="$2"; shift 2 ;;
      --time) walltime="$2"; shift 2 ;;
